...
- **Bucket 2 — analysis infrastructure:** CLOSED (2026-07-18). 1ST_READ fully segmented (raw ~3,626 functions — **corrected to ~3,187 in Bucket 3** after the boundary-detector over-seeding was found and the 555 suspects triaged; use ~3,187 for all forecasts; evidence-tagged, 84.9% instruction-covered, 4.1% unclassified); reproducible headless Ghidra generation (tools-local, never committed); five-state vocabulary live and machine-enforced (six states since Bucket 3); symbols file with provenance tags (21 entries); Azel pilot stopped at 19 names but established JP/US **address** identity (19/20 exact — but Bucket 3 showed call-graph *structure* does not transfer); Ymir savestate fixture confirmed load address 0x06006000 and a 440-byte runtime-write mask; overlay locate technique established for Bucket 4.
- **Bucket 3 — workflow at function scale:** CLOSED (2026-07-19). Library identification: 15 SBL/CPK catalogue members fingerprinted in 1ST_READ (5,056 B `library-identified`, 60 verified API symbols; sixth segment state `library-identified` added, machine-enforced). Per-function workflow formalized (docs/WORKFLOW.md; `tools/try_match.sh`; asm-differ adapted as `tools/fndiff.sh`). Calibration campaign on a representative 40-function sample: 1 exact match, 2 attempted (both drift-family #3), and the structural result (§4): only ~22.5% of small functions are matchable in isolation, ~77.5% need translation-unit reconstruction. Azel structural pilot (bounded): JP/US addresses transfer with high prior, but call-graph propagation into 1ST_READ did not transfer (yaz0r reimplemented behaviour, not code shape) — 2 confident names via dispatch tables; 141 overlay dispatch names banked for Bucket 4 (config/azel_overlay_dispatch.tsv). 555-suspect triage: 79% seed-artifacts, 0 genuine tiny functions; corrected inventory ~3,187. Permuter decision recorded (§4, conditional). Bucket 4 prerequisites established (see planned entry).
...
- **Bucket 4 — scale-out:** overlay-by-overlay; overlay similarity clustering (dedup — expected large win given PDS's templated overlays); progress tracking (simple JSON + badge first). **Prerequisites established by Bucket 3 (binding before scale-out):** (1) **function-boundary detector rewrite** — the current detector both over-seeds (bare `sts.l pr` tails and mid-function spill pushes; 17.5% of the campaign sample, 439 of the 555 suspects were artifacts) and under-seeds (misses save-less leaf functions). Fix approach: reachability-merge — compute each seed's flow extent (`tools/fn_extent.py`) and drop any seed interior to another's extent; additionally seed save-less leaves reached as call targets. (2) **translation-unit reconstruction** — cluster functions by `bsr` edges + shared-pool membership; match clusters, not functions. (3) the multi-fn match rate must be measured on reconstructed clusters, not estimated from isolated functions. Naming resource in hand: 141 Azel overlay dispatch names (config/azel_overlay_dispatch.tsv).
