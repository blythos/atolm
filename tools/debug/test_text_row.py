...
rgba_palette[0] = (64, 64, 64, 255)  # gray background
...
# tiles 0c0 to 0ca = 192 to 202
start_idx = 192
num_tiles = 11

img_width = num_tiles * 16
img_height = 16
rgba_rows = [bytearray() for _ in range(img_height)]

for tx in range(num_tiles):
    char_idx = start_idx + tx
    tile_offset = char_idx * 128
...
def _write_png(path, width, height, rgba_rows):
    ...
    raw = b''
    for row in rgba_rows: raw += b'\x00' + row
...
_write_png(r"e:\Dev\atolm\output\warning_text_row.png", img_width, img_height, [bytes(r) for r in rgba_rows])
print("Saved warning_text_row.png")
